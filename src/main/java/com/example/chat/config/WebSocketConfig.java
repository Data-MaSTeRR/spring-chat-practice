...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
     ...
     *    - 메모리 기반 Simple Message Broker 활성화
     *    - "/topic"으로 시작하는 destination을 브로커가 처리
     ...
     *
     * 프로덕션 환경 고려사항:
     * - Simple Broker 대신 외부 메시지 브로커 사용 권장
     *   (RabbitMQ, ActiveMQ, Redis 등)
     ...
     * - /queue/* : 1:1 메시지 (개인 메시지)
     */
    @Override
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        // 브로커 destination prefix 설정 (구독 경로)
